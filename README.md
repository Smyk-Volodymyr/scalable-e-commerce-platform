...
`PORT` у compose задано явно для кожного сервісу. Значення за замовчуванням у
схемах `order-service` і `payment-service` застаріле (3002), тож запускати їх
локально без `PORT` не варто.
